// repro_variant_return_14: 按任务要求 —— 把递归 lambda 换成命名空间作用域具名仿函数
// (operator() 模板按非包 alt 参数化, 返回具体变体 Ret), 但 switchOn 受约束变参重载集保持不变。
// 验证: 墙是否消失。(预期: 不消失, 因为墙在 switchOn 受约束变参模板本身, 与闭包无关。)
#include <variant>
#include <optional>
#include <utility>
#include <type_traits>
